...
void GUI::Init()
{
    ...
    WNDCLASS wc{};
    wc.lpszClassName = L"BoronWC";
    wc.lpfnWndProc = WndProc;
    RegisterClass(&wc);

    wchar_t buffer[67];
    swprintf_s(buffer,
               VersionInfo.EngineVersion >= 5.0 ? L"Boron (FN %.2f, UE %.1f)"
                                                : (VersionInfo.FortniteVersion >= 5.00 || VersionInfo.FortniteVersion < 1.2 ? L"Boron (FN %.2f, UE %.2f)" : L"Boron (FN %.1f, UE %.2f)"),
    ...
    while (!done)
    {
        ...
        ImGui::Begin("Boron", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
