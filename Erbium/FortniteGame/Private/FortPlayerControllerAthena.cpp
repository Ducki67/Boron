...
void AFortPlayerControllerAthena::ServerCheat(UObject* Context, FFrame& Stack)
{
    ...
    else
    {
        ...
        else if (command == "give")
        {
            ...
            std::string NameToken, RarityName;
            if (!BoronItems::Resolve(std::string(args[1].c_str()), NameToken, RarityName))
            {
                PlayerController->ClientMessage(FString(L"Unknown alias! Try: pump, tac, scar, ar, burst, bolt, heavysniper, hunting, deagle, smg, rpg, gl, minis, shield, medkit..."), FName(), 1.f);
                return;
            }
            ...
            if (!RarityName.empty())
            {
                ...
                {
                    auto& Pair = Names[i];
                    auto str = Pair.Key().ToString();
                    auto colcolIdx = str.find_last_of("::");
                    auto RealName = colcolIdx == -1 ? str : str.substr(colcolIdx + 1);

                    if (BoronItems::ToLower(std::string(RealName.c_str())) == BoronItems::ToLower(RarityName))
                    {
                        WantedRarity = Pair.Value();
                        break;
                    }
                }
            }

            auto TokenLower = BoronItems::ToLower(NameToken);
            const UFortItemDefinition* Best = nullptr;
            int BestScore = 0;
            ...
            for (int i = 0; i < TUObjectArray::Num(); i++)
            {
                ...
                if (WantedRarity != -1 && Item->Rarity != WantedRarity)
                    continue;

                FString Name = UKismetTextLibrary::Conv_TextToString(Item->HasDisplayName() ? Item->DisplayName : Item->ItemName);
                std::string NameStr = Name.GetData() ? BoronItems::ToLower(std::string(Name.ToString().c_str())) : std::string();

                int Score = 0;
                if (NameStr == TokenLower)
                    Score = 4;
                else if (!NameStr.empty() && NameStr.find(TokenLower) != std::string::npos)
                    Score = 2;
                else
                    continue;

                auto Path = UKismetSystemLibrary::GetPathName(Item).ToString();
                if (Path.find("Athena") != std::string::npos || Path.find("athena") != std::string::npos)
                    Score += 1;

                if (Score > BestScore)
                {
                    BestScore = Score;
                    Best = (const UFortItemDefinition*)Item;
                }
            }
            ...
            int ClipSize = 0;
            auto UncastedDef = (UFortItemDefinition*)Best;
            ...
            PlayerController->WorldInventory->GiveItem(Best, Count, ClipSize);
...
static void TryWeaponUpgrade(AFortPlayerControllerAthena* PC, uint8 InteractionByte)
{
    ...
    auto InteractingPawn = (AFortPlayerPawnAthena*)PC->Pawn;
    if (!InteractingPawn->CurrentWeapon)
        return;
    ...
    bool bIsSidegrading = (InteractionByte == 1);
    uint8 Direction = bIsSidegrading ? 2 : 1;
    ...
    int MetalToRemove = VersionInfo.FortniteVersion < 12 ? 150 : 200;

    int WoodCost = (int)Row->WoodCost * 50 - WoodToRemove;
    int StoneCost = (int)Row->BrickCost * 50 - StoneToRemove;
    int MetalCost = (int)Row->MetalCost * 50 - MetalToRemove;
    ...
    if (WoodCost < 0) WoodCost = 0;
    if (StoneCost < 0) StoneCost = 0;
    if (MetalCost < 0) MetalCost = 0;
    ...
    auto Deduct = [&](const UFortItemDefinition* MatDef, int Amount)
    {
        ...
        int NewCount = E->Count - Amount;
        if (NewCount <= 0)
        {
            FGuid MatGuid = E->ItemGuid;
            PC->WorldInventory->Remove(MatGuid);
        }
        else
        {
            E->Count = NewCount;
    ...
    auto MatCount = [&](const UFortItemDefinition* MatDef) -> int
    {
        if (!MatDef)
            return 0;
        auto E = Entries.Search([&](FFortItemEntry& En) { return En.ItemDefinition == MatDef; }, FFortItemEntry::Size());
    ...
    auto UpgradedDef = Row->UpgradedWeaponDef;

    auto IE = Entries.Search([&](FFortItemEntry& En) { return En.ItemDefinition == HeldDef; }, FFortItemEntry::Size());
    if (!IE)
        return;

    int LoadedAmmo = IE->LoadedAmmo;
    FGuid OldGuid = IE->ItemGuid;

    Deduct((const UFortItemDefinition*)Wood, WoodCost);
    Deduct((const UFortItemDefinition*)Stone, StoneCost);
    Deduct((const UFortItemDefinition*)Metal, MetalCost);
